...
1. One fundamental gap in most organisations testing strategy is "how to test prodction deployment before the release date".  In the normal process testing a system that is configured to be production is impossible (namespace violations at the least).  Through the use of a dynamically provisioned environment we can encapsulate the environment and test it as production.
